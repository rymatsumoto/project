...
volatile float pwm_out_BDN1 = 0.2;
...
volatile float proportionalGain = 0.1;
volatile float integralGain = 0.1;
...
void MW_main(void)
{
	...
	while(1)
	{
		...
		if (set_pwm_on_tx_pwmc == 1) {
			IPFPGA_write(BDN0, 0x07, 1);
			set_pwm_on_tx_pwmc = -1;
		}
		if (set_pwm_on_rx_pwmc == 1) {
			IPFPGA_write(BDN1, 0x07, 1);
			set_pwm_on_rx_pwmc = -1;
		}
	}
}
