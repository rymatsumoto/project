...
// PWMC制御設定
volatile float pwm_out_rx1_init = 0;
volatile float duty_step = 0;
volatile int iteration_end = 0;
int counter = 0;
int iteration = 0;
volatile int transmit_duration = 0;
volatile int transmit_interval = 0;
	...
	if (pwmc_control_on == 1)
	{
		if (iteration < iteration_end)
		{
			if (counter == 0)
			{
				...
				pwm_out_rx1 = pwm_out_rx1_init + duty_step * iteration;
			...
			else if (counter == transmit_duration)
			{
				PEV_inverter_stop_pwm(BDN_PEV);
				current_control_on = 0;

				IPFPGA_write(BDN_FPGA1, 0x07, 0);
				IPFPGA_write(BDN_FPGA2, 0x07, 0);
			}
			else if (counter == transmit_duration + transmit_interval)
			{
				counter = -1;
				iteration += 1;
			}
