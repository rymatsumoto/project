...
#define REF_PWMC_RX3 225

#define INCREMENT_RX1 1
#define DECREMENT_RX1 2
#define INCREMENT_RX2 3
#define DECREMENT_RX2 4
#define INCREMENT_RX3 5
#define DECREMENT_RX3 6
#define UPDATE_RX1_RX2_RX3 7
#define STOP 8
...
volatile int start_pwmc_rx3_init = 0;
...
float dc_current_increment_rx3 = 0;
float dc_current_decrement_rx3 = 0;
float gradient_rx1 = 0;
float gradient_rx2 = 0;
float gradient_rx3 = 0;
...
interrupt void read_dc_current(void)
{
    ...
    rx3_dc_current = (data[2] - 2.5) / Gth / 3;
}
...
interrupt void pwmc_control(void)
{
	...
	if (pwmc_control_on == 1) {
		...
		if (state == INCREMENT_RX1) {
		...
		else if (state == INCREMENT_RX3) {
			dc_current_decrement_rx2 = dc_current_updated;
			pwm_out_rx2 += DELTA;
			pwm_out_rx3 += DELTA;
			state = DECREMENT_RX3;
		}
		else if (state == DECREMENT_RX3) {
			dc_current_increment_rx3 = dc_current_updated;
			pwm_out_rx3 -= 2*DELTA;
			state = UPDATE_RX1_RX2_RX3;
		}
		else if (state == UPDATE_RX1_RX2_RX3) {
			dc_current_decrement_rx3 = dc_current_updated;
			pwm_out_rx3 += DELTA;
			...
			gradient_rx3 = (dc_current_increment_rx3 - dc_current_decrement_rx3) / (2 * DELTA);
			if (mwabs(gradient_rx1) < epsilon && mwabs(gradient_rx2) < epsilon && mwabs(gradient_rx3) < epsilon) {
				state = STOP;
			}
			else {
				pwm_out_rx1 += alpha * gradient_rx1;
				pwm_out_rx2 += alpha * gradient_rx2;
				pwm_out_rx3 += alpha * gradient_rx3;
				state = INCREMENT_RX1;
			}
		}
...
void initialize(void)
{
	...
	IPFPGA_write(BDN_FPGA3, 0x01, carrier_cnt_max);
	IPFPGA_write(BDN_FPGA3, 0x02, carrier_cnt_hlf);
	IPFPGA_write(BDN_FPGA3, 0x03, carrier_cnt_hlf);
	IPFPGA_write(BDN_FPGA3, 0x04, wref_rx3);
	IPFPGA_write(BDN_FPGA3, 0x05, DEAD_TIME / 10);
	IPFPGA_write(BDN_FPGA3, 0x09, REF_PWMC_RX3);
...
void MW_main(void)
{
	...
	while(1)
	{
		...
		if (start_pwmc_rx3_init == 1) {
			IPFPGA_write(BDN_FPGA3, 0x07, 1);
			start_pwmc_rx3_init = -1;
		}
