...
int pause_time_us = 0;
volatile int pause_time_thld_us = 1e4;
int pulse_time_us = 0;
volatile int pulse_time_thld_us = 1e3;
int transmit_time_us = 0;
volatile int transmit_time_thld_us = 3e6;
...
interrupt void state_control(void)
{
	...
	if (bench_mode == 0)
	{
		if (rpi_signal_prvs < RPI_SIGNAL_LOW && rpi_signal_crnt > RPI_SIGNAL_HIGH && init_time_us > 1e6)
		{
			servo_on = 1;
		}
	}
	else if (inverter_mode_rx == STANDBY)
	{
	...
	else if (inverter_mode_rx == PULSE)
	{
		pulse_time_us += TIMER_1_INTERVAL;
		if (pulse_time_us > pulse_time_thld_us)
		{
	...
	{
		if (i1_ampl > i1_ampl_thld_detect && init_time_us > 1e5)
		{
			if (inverter_start_count + 2e3 < encoder_count)
			{
...
void initialize(void)
{
	...
	C6657_timer0_init_vector(encoding_time_count, (CSL_IntcVectId)5);
	C6657_timer0_start();
